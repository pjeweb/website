...
class AdminUserController {
    ...
    private function getAllowedRoles() {
        ...
        return array_filter($roles, static function($v) use ($exclude) {
    ...
    public function adminUserEdit(array $params, ViewModel $model): string {
        ...
        $model->bans = $bans;
        $model->hasActiveBan = !empty($bans) && (count(array_filter($bans, static function($ban) { return $ban['active']; })) > 0);
    ...
    public function adminUserEditProcess(array $params): string {
        ...
        $allowChatting = $params['allowChatting'] ?? $user['allowChatting'];
        $allowNameChange = $params['allowNameChange'] ?? $user['allowNameChange'];
        $isTwitchSubscriber = $params['istwitchsubscriber'] ?? $user['istwitchsubscriber'];
        $discordName = $params['discordname'] ?? $user['discordname'];
        $discordUuid = $params['discorduuid'] ?? $user['discorduuid'];
        $minecraftName = $params['minecraftname'] ?? $user['minecraftname'];
        $minecraftUuid = $params['minecraftuuid'] ?? $user['minecraftuuid'];
    ...
    public function banUsers(array $params) {
        ...
        $end = !empty($params['duration']) ? Date::getSqlDateTimePlusSeconds('NOW', (int)$params['duration']) : null;
    ...
    public function removeBan(array $params): string {
        ...
        if (isset($params['follow']) and strpos($params['follow'], '/') === 0) {
    ...
    public function deleteUsers(array $params, Request $request) {
        ...
        $userService = UserService::instance();
        $authService = AuthenticationService::instance();
        $users = array_map(static function($v) use($userService) {
